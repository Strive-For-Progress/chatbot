...
def tagToProjects(tags=[],neg_tags=[],user_id=-1,applyEnd=False,projEnd=False,limit=5,order_by_match_num=True):
    ...
    if applyEnd: applyEnd_text = "AND NOW() < sp.apply_copi_end_date "
    else: applyEnd_text = ""

    if projEnd: projEnd_text = "AND NOW() < sp.end_date "
    else: projEnd_text = ""

    if order_by_match_num: order_text = "ORDER BY match_num DESC "
    else: order_text = "ORDER BY sp.start_date DESC " #新到舊
    ...
    sql=(
        "SELECT sp.id, sp.project_title, GROUP_CONCAT(r.name ORDER BY r.id ASC separator ', ' ) AS researches "
        "FROM specific_periods sp "
        "JOIN ( "
...
def user_prefer_project(user_id=-1,applyEnd=False,projEnd=False,limit=5):
...
def all_projects(applyEnd=False,projEnd=False,limit=5):
...
def all_researches():
    # 列出所有researches(在specific_period_researches使用中的)
    cursor = db.cursor(cursor=pymysql.cursors.DictCursor)
    sql = (
        "SELECT r.name FROM researches r "
        "JOIN specific_period_researches spr "
        "WHERE r.id = spr.research_id "
        "GROUP BY r.name "
        "ORDER BY r.id "
    )
...
def latest_projects(limit=5,applyEnd=False,projEnd=False):
...
if __name__ == "__main__":
    print("===================================")    
    for project in tagToProjects(tags=['C++','AI','DL'],neg_tags=['web']): print(project)
    print("===================================")
    for project in user_prefer_project(user_id=7, limit=2): print(project)
    print("===================================")
    for project in all_projects(limit=2): print(project)
    print("===================================")
